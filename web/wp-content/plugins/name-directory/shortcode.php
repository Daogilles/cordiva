...
function show_directory($attributes)
{
    ...
    {   
        $name_filter_let['character'] = $index_letter;
        $name_per_letter = get_directory_names($directory, $name_filter_let);
        $countNames = count($name_per_letter);
        if (empty($countNames)){
            echo ' <a class="name_directory_startswith inactive" href="#">' . strtoupper($index_letter) . '</a> ';
        }else {
